...
void programStart()
{
    ...
    if (n==1)
    {
        encrypt(str,n); //if the user enters 1 then call the encrypt function
    }
    else
    {
        encrypt(str,n); //if the user enters 2 then call the decrypt function
    }
    free(str);
}
...
void encrypt(char* str,int n)
{
    ...
    if (y==1) // if the user enters 1 we use XOR encryption
    {
        ...
        if(n==1)
       {
        ...
        printf("Do you want to encrypt? (1-yes,2-no):");}

        x=checkin();  //checking whether the user input is valid or not
        if (x==1) //if yes
        {
            if(n==1)
            printf("Decrypted phrase is: %s\n",XorEncrypt(xen,encryptor));//decrypt
            else
            printf("encrypted phrase is: %s\n",XorEncrypt(xen,encryptor));
